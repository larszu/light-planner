...
await page.evaluate((h) => window.__lp.setMode({ photo: true, heatmap: false, exposure: 1.2, haze: h }), HAZE);
await sleep(4500);

const shot = async (name, cam) => {
  await page.evaluate((c) => window.__lp.lookAt(c), cam);
  await sleep(1600);
  await page.screenshot({ path: `${OUT}/bm-${name}.png` });
...
await browser.close();
console.log('DONE', OUT, 'haze', HAZE);
