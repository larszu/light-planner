...
const STIL = join(WURZEL, 'src/App.css');
...
// ── 2. Kein Regler schrumpft sich per Stil-Attribut zurueck ──────────────
//
// Der zweite Weg, und der leisere: das Stilblatt bleibt richtig, aber an
// EINEM Regler haengt `style={{ height: 4 }}`. Ein Inline-Stil gewinnt gegen
// jeden Selektor, und der Regler ist wieder flach — nur faellt es niemandem
// auf, weil die anderen stimmen.
const dateien = [];
const gehe = (d) => {
...
/** `height: 4`, `height: '4px'` in einem Inline-Stil am Regler. */
