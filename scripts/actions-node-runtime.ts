 ...
 */
const NICHT_UNSER_PIN: Record<string, string> = {};
...
const urls = (referenz: string): string[] => {
...
const holen = async (referenz: string): Promise<string | null> => {
...
const pruefe = async (
  referenz: string,
  ueber: string | null,
): Promise<{ referenz: string; innen: string[] } | null> => {
...
try {
  let offen: { referenz: string; ueber: string | null }[] = alle.map((referenz) => ({ referenz, ueber: null }));
  while (offen.length > 0) {
    const naechste: { referenz: string; ueber: string | null }[] = [];
  ...
  console.log(`NICHT GEPRUEFT: die Actions waren nicht erreichbar (${(fehler as Error).message}).`)
